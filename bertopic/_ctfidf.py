...
class ClassTFIDF(TfidfTransformer):
    ...
    def fit(self, X: sp.csr_matrix, n_samples: int, idx=None, idx_c=None):
        ...
        if self.use_idf:
            ...
            if idx:
                ...
                print(f"------> for word _customer_ avg:{avg_nr_samples}, self.df:{self.df[idx_c]}")
                ...
                for idxx, df in enumerate(self.df):
                    if df > avg_nr_samples:
                        i += 1
                        words.append(idxx)
                print(f"------> {i} minus words, indices: {words}, bank's idx: {idx}")
